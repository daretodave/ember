...
describe('getProfileByUsername', () => {
  ...
  it('returns null when username not found', async () => {
    maybeSingle.mockResolvedValue({ data: null, error: null })
    // @ts-expect-error partial mock
    const result = await getProfileByUsername(mockSupabase, 'nobody')
    expect(result).toBeNull()
  })

  it('returns the profile row when username matches', async () => {
    const profile = {
      user_id: 'user-1',
    ...
    maybeSingle.mockResolvedValue({ data: profile, error: null })
    // @ts-expect-error partial mock
    const result = await getProfileByUsername(mockSupabase, 'marlowe')
    expect(result).toEqual(profile)
  })
